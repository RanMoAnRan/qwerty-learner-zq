...
interface DictChapterButtonProps {
  hideDictSwitch?: boolean
}

export const DictChapterButton = ({ hideDictSwitch = false }: DictChapterButtonProps) => {
        ...
        <Tooltip content="章节切换">
          <Listbox value={currentChapter} onChange={setCurrentChapter}>
            ...
            <Transition as={Fragment} leave="transition ease-in duration-100" leaveFrom="opacity-100" leaveTo="opacity-0">
              <Listbox.Options className="listbox-options z-10 w-auto min-w-[8.5rem] whitespace-nowrap">
                {range(0, chapterCount, 1).map((index) => (
                  <Listbox.Option key={index} value={index}>
                    {({ selected }) => (
                      <div className="group flex cursor-pointer items-center justify-between whitespace-nowrap">
                        {selected ? (
