...
function normalizeWord(rawWord: unknown): Word | null {
  if (!rawWord || typeof rawWord !== 'object') {
    return null
  }

  const data = rawWord as Record<string, unknown>
  ...
  if (Array.isArray(data.trans)) {
    trans = data.trans.filter((item): item is string => typeof item === 'string')
  } else if (data.trans === null || data.trans === undefined || typeof data.trans === 'object') {
    trans = []
  } else {
  ...
  return {
    name,
    trans,
    usphone: typeof data.usphone === 'string' ? data.usphone : '',
    ukphone: typeof data.ukphone === 'string' ? data.ukphone : '',
...
export function useWordList(): UseWordListResult {
  ...
  const words: WordWithIndex[] = useMemo(() => {
    ...
    // 过滤脏数据，避免因词典中异常词条导致页面崩溃。
    const normalizedWords: WordWithIndex[] = []
